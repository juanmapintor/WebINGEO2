...
function navBehaviour() {
    ...
    searchBehaviour();
    mobileNavBarBehaviour();
}
function mobileNavBarBehaviour() {
    ...
    for(let i = 0; i < expand.length; i++){
        ...
        expand[i].addEventListener('click', () =>{
            if (!open) {
                expand[i].setAttribute('style', 'transform: rotateX(-0.5turn)');
                dropItem[i].setAttribute('style','max-height: 1000px; overflow: visible;');
                open = true;
            } else {
                expand[i].setAttribute('style', 'transform: rotate(0turn)');
